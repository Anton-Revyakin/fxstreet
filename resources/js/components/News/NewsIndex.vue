                ...
                <div class="row align-items-center">
                    <div class="col text-center">
                        <router-link to="/twitter" class="text-decoration-none small" style="color: #1da1f2;">
                            <font-awesome-icon :icon="['fab', 'twitter']" style="font-size: 1.7rem;"/>
                            <br/>
                            Twitter
                        </router-link>
                    </div>
                    ...
                    <div class="col text-center">
                        <a href="#" @click.prevent="toggleSearch" class="text-white text-decoration-none small">
                            <font-awesome-icon :icon="['fas', 'search']" class="text-white" style="font-size: 1.7rem;"/>
                            <br/>
                            Поиск
                        </a>
                    </div>
                ...
                <div class="row h-100">
                        ...
                        <overlay-scrollbars class="height">
                            ...
                            <ul class="list-group" v-show="!showTitlesLoader">
                                <li class="border-bottom"
                                    ...
                                    :key="newsItem.id">
                                    <router-link :to="{name : 'news', params: {id : newsItem.id}}" :event="[loadingBody ? '' : 'click']" class="text-decoration-none">
                                        <div>
                                            <img :src="newsItem.image" alt="">
                                        </div>
                                        <div class="text-dark text-nowrap ellipsis border-info" :style="[!isShown(newsItem.id) ? {'border-right' : '4px solid'} : '']">
                                            <span v-html="newsItem.title"></span>
                                            <br>
                                            <span class="small text-black-50">{{ newsItem.published_at }}</span>
                                        </div>
                                    </router-link>
                                </li>
                            </ul>
                    ...
                    <div class="col-9" id="content">
                        <overlay-scrollbars class="height col">
                            <div :class="{'text-black-50':loadingBody}" v-if="Object.keys(newsBody).length > 0">
...
<script>
    ...
    export default {
        ...
        mounted() {
            ...
            this.getShown();
            this.getNewsTitles(() => {
                this.getNewsBodyLastShown();
            });

            setInterval(() => {
                if (this.searchText.length === 0) this.getNewsTitles();
            }, 30000);
        },
        methods : {
            /**
            ...
            isShown(newsId) {
                ...
                return storageShown[currDate].indexOf(newsId) !== -1;
            },
            /**
            ...
            /**
             * Привести список новостей в компактный вид
             *
             * @param list
             */
            ...
            /**
             * Отправить на последнюю новость
             */
            getNewsBodyLastShown() {
                if (undefined === this.$router.currentRoute.params.id) {
                    //Получить id последней просмотреной новости или первой новости за сегодня
                    let lastNewsId = storageShown[currDate][(storageShown[currDate].length - 1)] || this.newsList[(Object.keys(this.newsList).length - 1)].id;
                    //Если есть хоть какая-то новость - перенаправить на нее
                    if (undefined !== lastNewsId) this.$router.push({name: 'news', params: {id: lastNewsId}});
                } else {
                    this.getNewsBody(this.$router.currentRoute.params.id);
                }
            },
            /**
             ...
             */
            getNewsBody(newsId) {
                if (this.loadingBody) return false;
                ...
                axios.get('/news/' + newsId)
                    .then((response) => {
                        ...
                        if (!this.isShown(newsId)) this.saveShown(newsId);
                    });
            },
            /**
        ...
        watch   : {
            ...
            '$route'(to) {
                undefined !== to.params.id ? this.getNewsBody(to.params.id) : this.getNewsBodyLastShown();
            }
        }
    }
</script>
